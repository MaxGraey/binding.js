...
(function (global) {
    ...
    var setImmediate = window.setImmediate || (function () {
        ...
        var canPost = typeof window !== 'undefined' && window.postMessage && addEventListener;

        if (canPost) {
            var message = window.addEventListener ? 'message' : 'onmessage';
        ...
        return function (callback) {
            if (canPost) {
                queue.push(callback);
                window.postMessage('immediate-message-id', '*');
            } else {
