...
(function (global) {
    ...
    /**
     * WeakMap polyfill based on Andrea Giammarchi solution
     * @private 
     */
    ...
    /** @private */
    function hasPropertyAccessors(propertyDescriptor) { // has get or set
        return !!propertyDescriptor && (!!propertyDescriptor.get || !!propertyDescriptor.set);
    }
    ...
    function bindProperty(object, property, callback) {
        ...
        var srcDescriptor  = Object.getOwnPropertyDescriptor(object, property);
        var hasAccessors   = hasPropertyAccessors(srcDescriptor);
        var isConfigurable = isPropertyConfigurable(srcDescriptor);
        
        //console.log(property, hasAccessors, isConfigurable);
        
        if (!isConfigurable) {
            throw Error('Property "' + property + '" not configurable!');
        }
        
        if (hasAccessors) {
            throw Error('Property "' + property + '" with defined get/set accesors is not support in current version');
        }
    ...
    function bindHtmlElementProperties(object, properties, callback, options) {
        ...
        var options = Object.create(defaultObserveOptions_);      

        options.attributeFilter = properties;
        observer.observe(object, options);
        observers_.set(object, { observer: observer, options: options });
    }
